...
def _num_non_null_residues(seqres_to_structure_chain) -> int:
    """Ported from the official esm repo (biohub/esm)."""
    return sum(
...
def chain_to_ndarray(
    ...
    """Convert an mmCIF chain to atom37 ndarrays, aligned to the SEQRES sequence.

    Ported from the official esm repo (biohub/esm). Unlike the PDB-format-based
    conversion, this path imposes no single-character chain-id or B-factor column
    width limits.
    """
    entity_id = None
...
class ProteinChain:
    ...
    def from_mmcif(
        ...
        Ported from the official esm repo (biohub/esm). Reads the structure
        natively via MmcifWrapper without going through the PDB format, so
        multi-character chain ids and wide B-factor columns are supported.

        If neither `chain_id` nor `entity_id` is specified, defaults to the
        longest chain of the first entity.
        """
        ...
        if entity_id is not None:
            ...
            # Select the chain id corresponding to the longest chain.
            chain_id = max(
                chains,
        ...
        (
            ...
            confidence,
            _,
        ) = chain_to_ndarray(mmcif.structure, mmcif, chain_id, is_predicted)
    ...
    def chain_iterable_from_mmcif(
        ...
        """Yield a ProteinChain for each polypeptide chain in an mmCIF file,
        in file order. Ported from the official esm repo (biohub/esm)."""
        if isinstance(path, MmcifWrapper):
            mmcif = path
        else:
            mmcif = MmcifWrapper.read(path, id)
