...
def build_affine3d_from_coordinates(
    ...
    # # Accept coords with optional leading prompt dimension(s).
    # # If coords has 5 dims (e.g. (P, B, L, CA, C) or (1, B, L, CA, C)),
    # # collapse the leading dims into a single batch dimension so the
    # # downstream code (which expects (B, L, CA, C)) works correctly.
    # original_shape = tuple(coords.shape)
    # if coords.dim() == 5:
    #     # Merge all leading dims except the last 3 into a single batch dim
    #     lead = int(torch.prod(torch.tensor(coords.shape[:-3])).item())
    #     coords = coords.reshape(lead, *coords.shape[-3:])
    ...
    # Diagnostic prints: help debug shape mismatch between coords and coord_mask
    # try:
    #     print(f"[affine3d DEBUG] original_shape={original_shape} -> normalized coords.shape={coords.shape}, coord_mask.shape={coord_mask.shape}, coords.device={coords.device}, coords.dtype={coords.dtype}")
    #     nm = (~coord_mask[..., None, None])
    #     print(f"[affine3d DEBUG] ~coord_mask[...,None,None].shape={nm.shape}, coord_mask.sum(-1).shape={coord_mask.sum(-1).shape}")
    # except Exception as _e:
    #     print("[affine3d DEBUG] failed to print debug info:", _e)
